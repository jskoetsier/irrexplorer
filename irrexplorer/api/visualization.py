...
from irrexplorer.api.caching import cached_api_call
...
async def get_prefix_allocation_data(request: Request) -> JSONResponse:
    ...
    Returns aggregated prefix allocations by RIR and ASN,
    suitable for tree maps or bubble charts.
    """
    database = request.app.state.database

    cache_key = "viz:prefix_allocation"
    ...
    result = await cached_api_call(cache_key, fetch_data, ttl=3600)
...
async def get_asn_relationships(request: Request) -> JSONResponse:
    ...
    Returns nodes (ASNs) and edges (relationships) based on
    prefix overlaps and IRR data.
    """
    ...
    cache_key = f"viz:asn_relationships:{asn_int}"
    ...
    async def fetch_data():
        ...
        related_rows = await database.fetch_all(
            overlapping_asns_query, {"asn": asn_int}
        )
    ...
    result = await cached_api_call(cache_key, fetch_data, ttl=1800)
    return JSONResponse(result)
...
async def get_historical_timeline(request: Request) -> JSONResponse:
    ...
    Returns time-series data of queries and popular searches.
    """
    database = request.app.state.database
    ...
    async def fetch_data():
        ...
        timeline_rows = await database.fetch_all(
            timeline_query, {"since_date": since_date}
        )
        ...
        timeline_data = defaultdict(
            lambda: {"asn": 0, "prefix": 0, "set": 0, "total": 0}
        )
    ...
    result = await cached_api_call(cache_key, fetch_data, ttl=900)
    return JSONResponse(result)
...
async def get_rir_distribution(request: Request) -> JSONResponse:
    ...
    Returns RIR allocation statistics with geographical context.
    """
    database = request.app.state.database

    cache_key = "viz:rir_distribution"
...
async def get_prefix_size_distribution(request: Request) -> JSONResponse:
    ...
    Shows distribution of prefix sizes (CIDR mask lengths).
    """
    database = request.app.state.database

    cache_key = "viz:prefix_distribution"
