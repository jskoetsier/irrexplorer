...
async def _get_rpki_dashboard_data(database):
    ...
    # Get RPKI validation status counts
    rpki_stats_query = """
        SELECT
            COALESCE(rpki_status, 'unknown') as status,
            COUNT(*) as count,
    ...
    rpki_rows = await database.fetch_all(rpki_stats_query)

    # Get ROA coverage by RIR
    roa_coverage_query = """
        SELECT
            r.rir::text as rir,
            COUNT(DISTINCT r.prefix) as total_prefixes,
            COUNT(DISTINCT CASE WHEN b.rpki_status = 'valid' THEN r.prefix END) as valid_prefixes,
            COUNT(DISTINCT CASE WHEN b.rpki_status = 'invalid' THEN r.prefix END) as invalid_prefixes,
            COUNT(DISTINCT CASE WHEN b.rpki_status = 'not_found' THEN r.prefix END) as not_found_prefixes
        FROM rirstats r
        LEFT JOIN bgp b ON r.prefix = b.prefix
        GROUP BY r.rir
    """

    roa_rows = await database.fetch_all(roa_coverage_query)
    ...
    for status in status_breakdown:
        if total_prefixes > 0:
            status_breakdown[status]["percentage"] = round(
                (status_breakdown[status]["count"] / total_prefixes) * 100, 2
            )
        else:
            status_breakdown[status]["percentage"] = 0
    ...
    for row in roa_rows:
        ...
        if total > 0:
            roa_coverage.append({
                ...
                "valid": row["valid_prefixes"] or 0,
                "invalid": row["invalid_prefixes"] or 0,
                "not_found": row["not_found_prefixes"] or 0,
                "coverage_percentage": round(
                    ((row["valid_prefixes"] or 0) / total) * 100, 2
                )
            })
    ...
    if asn:
        ...
        coverage_query = """
            SELECT
                ...
                b.rpki_status,
                CASE
                    WHEN b.rpki_status = 'valid' THEN true
                    ELSE false
                END as has_roa
            FROM bgp b
            WHERE b.asn = :asn
        ...
        prefixes = []
        total = len(rows)
        covered = sum(1 for r in rows if r["has_roa"])
        ...
        global_query = """
            SELECT
                ...
                COUNT(CASE WHEN rpki_status = 'valid' THEN 1 END) as covered,
                COUNT(CASE WHEN rpki_status = 'invalid' THEN 1 END) as invalid,
                COUNT(CASE WHEN rpki_status = 'not_found' THEN 1 END) as not_covered
            FROM bgp
        """
        ...
        return {
            "total_prefixes": total,
            "covered_prefixes": row["covered"],
            "invalid_prefixes": row["invalid"],
            "not_covered_prefixes": row["not_covered"],
            "coverage_percentage": round((row["covered"] / total * 100), 2) if total > 0 else 0,
            "timestamp": datetime.utcnow().isoformat()
        }
...
async def _get_irr_consistency_data(database, asn: Optional[int] = None):
    ...
    if asn:
        ...
        total = row["total_routes"]
        consistent = row["consistent_routes"]
...
async def _get_hijack_detection_data(database):
    ...
    # Find prefixes with RPKI invalid status (potential hijacks)
    hijack_query = """
        SELECT
            b.prefix::text as prefix,
            b.asn as announcing_asn,
            b.rpki_status,
            r.asn as authorized_asn
        FROM bgp b
        LEFT JOIN roas r ON b.prefix = r.prefix
        WHERE b.rpki_status = 'invalid'
        ORDER BY b.prefix
        LIMIT 100
    """

    rows = await database.fetch_all(hijack_query)

    alerts = []
    for row in rows:
        alerts.append({
            "prefix": row["prefix"],
            "announcing_asn": row["announcing_asn"],
            "authorized_asn": row["authorized_asn"],
            "severity": "high",
            "description": f"AS{row['announcing_asn']} is announcing {row['prefix']} but ROA authorizes AS{row['authorized_asn']}"
        })

    # Get recent changes (simulated - would need historical data)
    recent_count = len(alerts)

    return {
        "total_alerts": recent_count,
        "high_severity": recent_count,
        "medium_severity": 0,
        "low_severity": 0,
        "alerts": alerts,
        "timestamp": datetime.utcnow().isoformat()
